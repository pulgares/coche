...
try:
    ...
    print "No he podido conectarme al puerto "
...
def Radio():
    ...
    otra_ventana4.title('Radio')

    buttonr1 = tk.Button(otra_ventana4, text="subefrecuencia",command=subefrecuencia)
    buttonr1.place (x=10, y=10)
    buttonr1.config( height = 10, width = 20 )#como aumento el numero cuando pulso el boton-?
    root.iconify()

    buttonr2 = tk.Button(otra_ventana4, text="bajafrecuencia",command=bajafrecuencia)
    buttonr2.place (x=10, y=250)
    buttonr2.config( height = 10, width = 20 )

    buttonr3 = tk.Button(otra_ventana4, text="ENTER",command=Enter)
    buttonr3.place (x=410, y=100)
    buttonr3.config( height = 6, width = 20 )

    buttonr4 = tk.Button(otra_ventana4, image=imagenatras,fg="White",command=lift_win1)
    buttonr4.place (x=410, y=300)
    buttonr4.config( height = 96, width = 96 )
    buttonr4.config(bg="black")

    Texto = Text(otra_ventana4, height=3, width=30)
    Texto.place(x=10, y=190)

#   como meto el valor de la frecuencia al pulsar los botones?
    Texto.insert(END, contador,'big')
    Texto.tag_configure('big', font=('Verdana', 20, 'bold'))
def Enter():
	print"frecuencia"# como se lo paso al arduino una vez pulso enter?

def subefrecuencia():

    arduino.write('d')#como calculo aqui el valor de la pulsacion de los botones?
    print ('Valor de contador: ' + repr (contador+1))

def bajafrecuencia():
    arduino.write('c')
    print "bajafrecuencia"
...
def Sensores():
    ...
    lblimagen=Label(otra_ventana3, image=imagensensor).place(x=150, y=0)
